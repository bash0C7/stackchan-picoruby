...
class WavTest < Test::Unit::TestCase
  ...
  def test_parses_data_and_format
    ...
    assert_equal W::SAMPLE_RATE, fmt.sample_rate
  ...
  def test_expect_mono_s16_passes_and_rejects
    ok = W.parse(build_wav([1].pack("s<*"))).first
    assert_equal ok, W.expect_mono_s16!(ok)

    bad = W.parse(build_wav([1].pack("s<*"), rate: W::SAMPLE_RATE / 2)).first
    assert_raise(W::FormatError) { W.expect_mono_s16!(bad) }
  end
