...
class CalibrationMedianTest < Test::Unit::TestCase
  def test_median_of_odd_count
    assert_equal 485, Stackchan::BLE::Calibration.median([482, 485, 487])
  end

  def test_median_of_even_count_uses_lower_middle
    assert_equal 484, Stackchan::BLE::Calibration.median([482, 484, 486, 488])
  end

  def test_median_single_value
    assert_equal 500, Stackchan::BLE::Calibration.median([500])
  end

  def test_median_empty_raises
    assert_raise(ArgumentError) { Stackchan::BLE::Calibration.median([]) }
  end
end
...
class CalibrationComputeAnchorsTest < Test::Unit::TestCase
  ...
  def test_symmetric_ranges
    result = Stackchan::BLE::Calibration.compute_anchors(
...
class CalibrationClassifyVerifyTest < Test::Unit::TestCase
  def test_pass_within_three
    result = Stackchan::BLE::Calibration.classify_verify(yaw_delta: 2, pitch_delta: -3)
    assert_equal :pass, result
  end

  def test_warn_above_three_within_ten
    result = Stackchan::BLE::Calibration.classify_verify(yaw_delta: 5, pitch_delta: 0)
    assert_equal :warn, result
  end

  def test_warn_when_only_pitch_exceeds
    result = Stackchan::BLE::Calibration.classify_verify(yaw_delta: 1, pitch_delta: -8)
    assert_equal :warn, result
  end

  def test_fail_above_ten
    result = Stackchan::BLE::Calibration.classify_verify(yaw_delta: 12, pitch_delta: 0)
...
class CalibrationFormatTest < Test::Unit::TestCase
  ...
  def test_format_ruby
    out = Stackchan::BLE::Calibration.format(anchors, :ruby)
  ...
  def test_format_env
    out = Stackchan::BLE::Calibration.format(anchors, :env)
    assert_equal(
  ...
  def test_format_json
    out = Stackchan::BLE::Calibration.format(anchors, :json)
  ...
  def test_format_unknown_raises
    assert_raise(ArgumentError) { Stackchan::BLE::Calibration.format(anchors, :yaml) }
  end
end

class CalibrationParseRawDetailTest < Test::Unit::TestCase
  def test_parse_numeric_pair
    pose = Stackchan::BLE::Calibration.parse_raw_detail("<yaw_raw:485,pitch_raw:628>\n")
    assert_equal({ yaw_raw: 485, pitch_raw: 628 }, pose)
  end

  def test_parse_unknown_yaw
    pose = Stackchan::BLE::Calibration.parse_raw_detail("<yaw_raw:unknown,pitch_raw:628>\n")
    assert_nil pose[:yaw_raw]
    assert_equal 628, pose[:pitch_raw]
  end

  def test_parse_both_unknown
    pose = Stackchan::BLE::Calibration.parse_raw_detail("<yaw_raw:unknown,pitch_raw:unknown>\n")
    assert_nil pose[:yaw_raw]
    assert_nil pose[:pitch_raw]
  end

  def test_parse_negative_value
    pose = Stackchan::BLE::Calibration.parse_raw_detail("<yaw_raw:-27139,pitch_raw:628>\n")
    assert_equal(-27139, pose[:yaw_raw])
  end

  def test_parse_malformed_raises
    assert_raise(ArgumentError) { Stackchan::BLE::Calibration.parse_raw_detail(".\n") }
  end
end
...
class CalibrationSampleTest < Test::Unit::TestCase
  ...
  def test_sample_pose_returns_median_of_n_reads
    ...
    pose = Stackchan::BLE::Calibration.sample_pose(client, samples: 3)
  ...
  def test_sample_pose_raises_when_any_unknown
    ...
    assert_raise(Stackchan::BLE::Calibration::UnknownReadError) do
      Stackchan::BLE::Calibration.sample_pose(client, samples: 3)
    end
  end

  def test_sample_pose_single_sample
    client = FakeClient.new(["<yaw_raw:500,pitch_raw:620>\n"])
    pose = Stackchan::BLE::Calibration.sample_pose(client, samples: 1)
...
class CalibrationAlignOnlyTest < Test::Unit::TestCase
  ...
  def test_run_align_only_sends_torque_off_prompts_then_torque_on
    ...
    prompts = []
    Stackchan::BLE::Calibration.run_align_only(
      client: client,
...
class CalibrationFullRunTest < Test::Unit::TestCase
  ...
  def test_run_full_calibrate_happy_path_returns_anchors_outcome_pass
    ...
    result = Stackchan::BLE::Calibration.run_full_calibrate(
      client: client,
      prompt: ->(_msg) { nil },
      stdout: StringIO.new,
      samples: 1,
  ...
  def test_run_full_calibrate_engage_torque_sends_torque_on_at_end
    ...
    Stackchan::BLE::Calibration.run_full_calibrate(
      client: client,
      prompt: ->(_msg) { nil },
      stdout: StringIO.new,
      samples: 1,
