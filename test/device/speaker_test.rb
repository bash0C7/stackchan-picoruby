class SpeakerTest < Picotest::Test
  # ITU G.711 mu-law decode. Known vectors: 0xFF/0x7F -> 0, 0x00 -> -32124, 0x80 -> +32124.
  def test_ulaw_zero_codes
    assert_equal "\x00\x00", Speaker.ulaw_decode("\xFF")
    assert_equal "\x00\x00", Speaker.ulaw_decode("\x7F")
  end

  def test_ulaw_extremes
    assert_equal "\x84\x82", Speaker.ulaw_decode("\x00")  # -32124 -> 0x8284, LE 84 82
    assert_equal "\x7C\x7D", Speaker.ulaw_decode("\x80")  # +32124 -> 0x7D7C, LE 7C 7D
  end

  def test_ulaw_two_bytes_per_code
    # bytesize, not length: host PicoRuby VM mis-counts multibyte String#length
    assert_equal 6, Speaker.ulaw_decode("\x00\x80\xFF").bytesize
  end

  # AW88298 init writes (from M5Unified), 16-bit big-endian -> [reg, hi, lo] triples.
  def test_aw88298_init_writes_8khz
    seq = Speaker.aw88298_init_writes(8000)
  ...
  def test_play_ulaw_feeds_decoded_pcm_to_i2s
    spk = Speaker.new(i2c: FakeI2C.new, i2s: I2S.new(sample_rate: 8000))
