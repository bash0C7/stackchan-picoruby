...
- **レイテンシ計測**: `tools/latency_baseline.zsh`（wrapper込みwall clock、`LOG=`で出力先）→ `ruby tools/latency_summary.rb <log>`でverbごとの中央値 / p90 / failures。device側は1コマンドごとに`[t] rx=<us> ack=<us> d=<us>`（serial）、Mac側はdaemon.logに`[t] <frame> ack=<ms>ms[ detail=<ms>ms]`（timeout時は`ack=timeout` / `detail=timeout`）。現状値と内訳はvault `02_dev_docs/stackchan-picoruby/review/2026-08-30-latency-investigation.md` §6。face別の内訳は`ROUNDS=8 tools/face_profile.zsh`（labelにface名を残すので`latency_summary.rb`がface単位で集計する）。
- **LCD描画のコストは`SPI#write`の呼び出し回数で決まる**。実機回帰でlatency = 0.181s + 0.853ms×calls（R²=0.991、interceptは無描画の底値0.175sと一致）。RAMWRトランザクション数で当てるとR²=0.990だがinterceptが24ms過大、送信バイト数はR²=0.118で何も説明しない。**1回の`SPI#write`は中身の大小にほぼ関係なく約0.85msかかる**（SPIは40MHzなので線路時間ではない）。速くする手段は塗る面積を減らすことではなく呼び出し回数を減らすことで、多数の短いspanからなる描画は`ILI9342#batch`で1枚のバッファにまとめる
- **描画コストの計測にdevice側の`puts`を使わない**。`ruby tools/face_spi_cost.rb`が実ドライバをカウンタ付きSPIで空回しし、face別の`SPI#write`回数・RAMWR数・予測latencyをホストだけで出す。device側に一時ログを足すとboot loopに入り、USB抜き差しでしか復旧できない
