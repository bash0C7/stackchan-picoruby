...
- **`BLE#start` は `@event_queue`（`Task::Queue`）を blocking `pop` する単一ループに統合済み**（`pop_packet` / `pop_heartbeat` / `BLE::POLLING_UNIT_MS` は廃止 API）。`peri.start`（引数なし）を呼べば `timeout_ms=nil, stop_state=:no_stop` で無限ループ + 正しい dispatch がそのまま手に入るので、**独自の `start` override は原則不要**。override すると `@event_queue.pop` が呼ばれなくなり、ESP32 port の `picoruby_nimble_dequeue_event`（evq drain）→ `BLE_push_event` を含む host イベント処理が VM thread 上で完全停止する（`advertise()` すら効かなくなる。`app/application.rb` の custom `start` で実際に踏み、2026-08-18 に除去済み）。blocking 中（音声再生等）に per-tick で drain だけしたい場合は non-blocking 版 `event = @event_queue.pop(timeout_ms: 0); _event_popped if event; packet_callback(event) if event.is_a?(String)` を手書きする（`pop_and_dispatch` パターン、`pc/stackchan-pico/app/ble_client.rb` 参照） 2026-08-30 以降の `app/application.rb` は `BLE#start` を使わず `StackChanApp#run` (`StackchanApp::LinkLoop` の 20 ms tick、毎 tick `_event_popped`) で回している。`start` を使わない理由は「`_event_popped` は pop が event を返した時しか呼ばれず、queue を起こせるのは 1 s heartbeat だけ」なので、override ではなく loop ごと置き換えている。
...
- **レイテンシ計測**: `tools/latency_baseline.zsh` (wrapper 込み wall clock、`LOG=` で出力先) → `ruby tools/latency_summary.rb <log>` で verb ごとの中央値 / p90 / failures。device 側は 1 コマンドごとに `[t] rx=<us> ack=<us> d=<us>` (serial)、Mac 側は daemon.log に `[t] <frame> ack=<ms>ms[ detail=<ms>ms]`。合格線 (Phase 1): face 中央値 ≤0.8 s、p90 ≤1.0 s、failures 0。
...
3. `pop_packet` / `pop_heartbeat` / `BLE::POLLING_UNIT_MS` を直呼びする全ファイル（`grep -rn "pop_packet\|pop_heartbeat\|POLLING_UNIT_MS"`。`app/application.rb` と `pc/stackchan-pico/app/ble_client.rb` が主な該当）を上の Task::Queue パターンに migrate する
