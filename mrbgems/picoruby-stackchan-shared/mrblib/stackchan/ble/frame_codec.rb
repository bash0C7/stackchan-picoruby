module Stackchan::BLE
  module FrameCodec
    ...
    # NOTE: bare `module_function` (no args) is a no-op on PicoRuby — it does
    # NOT promote the following defs to module-level methods. Use the explicit
    # `module_function :sym, ...` form at the end of the module instead (works
    # identically on CRuby and PicoRuby).
    ...
    module_function :encode_face, :encode_led, :encode_head, :encode_torque,
                    :encode_selftest, :encode_read_pos, :parse_ack,
                    :touch_event?, :parse_touch, :encode_pairs
  end
end
